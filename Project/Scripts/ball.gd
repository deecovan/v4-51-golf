...
func _integrate_forces(state):
	...
	if state.linear_velocity.length() < low_crit * 2:
		stopped.emit()
...
func is_lowspeed(speed: Vector3) -> bool:
	return speed.length() < low_speed and speed.y < low_crit

## Add LERP friction simulation for the ball at low speeds
func add_friction(speed: Vector3) -> Vector3:
	speed[0] = lerp(speed[0], 0.0, low_crit)
	speed[1] = lerp(speed[1], 0.0, low_crit)
	speed[2] = lerp(speed[2], 0.0, low_crit)
	return speed
